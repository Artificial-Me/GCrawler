...
from ghostcrawler_bike_specs import (
...
class SystemMonitor(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        self.cpu_progress = ttk.Progressbar(
            ...
            maximum=100, style="Redline.Horizontal.TProgressbar"
        )
...
class GhostCrawlerGUI:
    ...
    def apply_brand_colors(self):
        ...
        style.configure("Redline.Horizontal.TProgressbar", 
                       troughcolor=CEMENT, 
                       background=REDLINE,
                       bordercolor=ASPHALT,
                       lightcolor=REDLINE,
    ...
    def setup_control_buttons(self, parent):
        ...
        self.start_button = ttk.Button(
            ...
            style="Redline.TButton", width=20
        )
    ...
    def load_settings(self):
        ...
        try:
            ...
            # Apply settings - only set variables that exist
            self.input_file_var.set(settings.get("input_file", ""))
            self.output_dir_var.set(settings.get("output_dir", "Specs"))
            
            # Only set UI variables if they exist (after setup_ui has been called)
            if hasattr(self, 'browsers_var'):
            ...
            # Only log error if console exists
            if hasattr(self, 'console'):
    ...
    def run_crawler(self):
        ...
        try:
            # Redirect print statements to GUI console
            import io
            import contextlib
            ...
            # Run async crawler with captured output
            with capture_stdout() as captured:
                ...
                # Send initialization output to GUI
                init_output = captured.getvalue()
                if init_output:
                    for line in init_output.strip().split('\n'):
                        if line.strip():
                            self.message_queue.put(("log", line.strip(), "info"))
                    captured.seek(0)
                    captured.truncate(0)
                ...
                while not crawl_task.done():
                    ...
                    # Check for new console output every 0.2 seconds
                    current_time = time.time()
                    if current_time - last_output_check >= 0.2:
                        output = captured.getvalue()
                        if output:
                            # Send new output lines to GUI
                ...
                final_output = captured.getvalue()
                if final_output:
