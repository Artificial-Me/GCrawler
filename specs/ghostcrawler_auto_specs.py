...
import asyncio, os, json, logging, time, gc
...
from url_processor import URLProcessor, check_and_filter_urls, filter_urls_from_file
...
class CrawlerConfig:
    ...
    retry_incomplete_data: bool = True
...
class DataExtractor:
    @staticmethod
    async def extract_car_specs(page: Page) -> Dict[str, Any]:
        data = {}
        ...
        try:
                    ...
                    try:
                        ...
                        if content:
                            ...
                            if isinstance(parsed, dict):
                                ...
                                if data_type == 'Car':
                                    jsonld_data['car'] = parsed
                                    print(f"         ✓ Found Car data in script {i+1}")
            ...
            image_elem = page.locator('div#car_image img')
            if await image_elem.count() > 0:
                data['image_url'] = await image_elem.get_attribute('src')
            
            print(f"      🔍 Extracting key specs...")
            key_specs_selectors = [
                'div.col-md-6 > div.col-12 > h3.posts_title:has-text("Key Specs")/..',
                'h3:has-text("Key Specs")/..',
                'div:has(> h3:has-text("Key Specs"))'
            ]
            ...
            print(f"      🔍 Extracting detailed specs...")
...
class HTMLSaver:
    ...
    async def save_html(self, url: str, data: Dict[str, Any]) -> Optional[str]:
        ...
        if not all([manufacturer, id_part, filename]):
            ...
            return None
        raw_html_dir = self.base_dir / manufacturer / f"{manufacturer.upper()}_RAW_HTML"
        raw_html_dir.mkdir(parents=True, exist_ok=True)
        output_path = raw_html_dir / f"{id_part}-{filename}"
        async with aiofiles.open(output_path, 'w', encoding='utf-8') as f:
            await f.write(self._build_html_content(data))
        self.files_saved += 1
        logger.info(f"Saved file {self.files_saved}: {output_path.name}")
...
"""
        ...
        if 'jsonld' in data:
            ...
            if 'car' in data['jsonld']:
                html += '<div id="car_jsonld">\n<h3>Car Data</h3>\n'
                html += f'<pre>{json.dumps(data["jsonld"]["car"], indent=2)}</pre>\n'
                html += '</div>\n'
...
class GhostCrawler:
    ...
    async def crawl_url(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"   📊 Extracting car specifications...")
            data = await DataExtractor.extract_car_specs(page)
